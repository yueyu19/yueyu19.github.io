...
In other words, as the iteration number $k$ increases, the distance between $x^k$ and $x^\star$ is upper bounded by a number that converges to zero exponentially. Hence said distance must converge to zero. 
...
======
  ...
  Now that you have the skeleton key to unlock all of these proofs, and many more out there. Use it well!
