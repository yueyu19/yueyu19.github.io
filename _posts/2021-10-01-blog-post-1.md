...
tags:
  ...
  - convex optimization
---
Do you remeber the law of cosines from your high-school math classes? Did you know that it is an incredibly useful technique in the convergence proofs of optimization methods? 
...
\begin{equation}
\langle x^1-x^2, x^3-x^1\rangle=\frac{1}{2}||x^3-x^2||_2^2-\frac{1}{2}||x^3-x^1||_2^2-\frac{1}{2}||x^1-x^2||_2^2.
\end{equation}

Notice that the above equation is indeed equivalent to the law of cosines. In particular, $x^1, x^2$ and $x^3$ are the coordinates of the three vertices of the triangle. The length of each side of the triangle is measured by the Euclidean distance between each pair of vertices. Finally, the cosine is now hidden in the inner product term. 
...
* **Difference of two quadratic functions**: Many convergence proofs rely on showing certain <em>quadratic disatance<em> to the optimizer is decreasing. Hence the convergence proof usually starts by showing the difference of two quadratic functions is negative. 
...
We also make the following assumptions on function $f$: there exists $0< \mu\leq \lambda$ such that
  ...
  $$\frac{\mu}{2}||x-x'||_2^2\leq B_f(x, x')\leq \frac{\lambda}{2}||x-x'||_2^2$$
...
**Aha!** Now we see some inner product terms similar to those in the law of cosines! By applying equation (1) and equation (2) to **the triangle defined by $x^k, x^{k+1}$ and $x^\star$**, we can show the following:
  ...
  $$\langle x^{k+1}-x^k, x^\star-x^{k+1}\rangle=\frac{1}{2}||x^k-x^\star||_2^2-\frac{1}{2}||x^{k+1}-x^\star||_2^2-\frac{1}{2}||x^{k+1}-x^k||_2^2,$$
...
Furthermore, by using equation (5), we can immediately obtain the following inequalities:
  ...
  $$\alpha B_f(x^{k+1}, x^k)\leq \frac{\alpha \lambda}{2}||x^{k+1}-x^k||_2^2,$$
  
  $$\frac{\alpha \mu}{2}||x^k-x^\star||_2^2\leq \alpha B_f(x^\star, x^k),$$
  
  $$\frac{\alpha \mu}{2}||x^{k+1}-x^\star||_2^2\leq \alpha B_f(x^{k+1}, x^\star).$$
...
Finally, by summing up equation (8-13), and letting $\alpha\leq\frac{1}{\lambda}$, we obtain the following result:
  ...
  $$\frac{1+\frac{\mu}{\lambda}}{2}||x^{k+1}-x^\star||_2^2\leq \frac{1-\frac{\mu}{\lambda}}{2}||x^k-x^\star||_2^2.$$

### Proving Exponential convergence by induction  
  
By using the above relation recursively, we can show the following
  
  $$||x^{k+1}-x^k||_2^2\leq \frac{\lambda-\mu}{\lambda+\mu}||x^k-x^\star||_2^2\leq \cdots \leq \left(\frac{\lambda-\mu}{\lambda+\mu}\right)^k||x^1-x^\star||_2^2.$$
...
======
 ...
 I also use the law of cosines in all of my published work on distributed and conic optimization methods, see some recent examples [here](https://arxiv.org/pdf/2009.06980.pdf) and [here](https://arxiv.org/pdf/2108.10260.pdf), and my [list of publications](/_pages/pub_topic.md) for more references. Unlike the simple gradient descent method above, the primal-dual methods I developed in these papers require more complicated proofs, where I occasionally need to use the law of cosines four times in a row. Without the law of cosines, I would have need to complete twelve different squares manually! 
